...
impl GameEngine {
    ...
    pub fn initialize_pieces(&mut self) {
        [1, 3, 5, 7, 0, 2, 4, 6, 1, 3, 5, 7]
            ...
            .for_each(|(x, y)| {
                self.board[x][y] = Some(GamePiece::new(PieceColor::Red));
            });
    ...
    pub fn move_piece(&mut self, mv: &Move) -> Result<MoveResult, ()> {
        ...
        // cheater unwrap(), we should validate this instead, but we can expect the values here now
        ...
        let crowned = if self.is_coronal(piece, mv.to) {
    ...
    // Black pieces in row 0 or Red pieces in row 7 are crowned
    fn is_coronal(&self, piece: GamePiece, coord: Coordinate) -> bool {
        let Coordinate(_x, y) = coord;

        (y == 0 && piece.color == PieceColor::Black) || (y == 7 && piece.color == PieceColor::Red)
    }
    ...
    fn valid_move(&self, p: &GamePiece, from: &Coordinate, to: &Coordinate) -> bool {
        if !to.on_board() || !from.on_board() {
            ...
            if let Some(_piece) = self.board[tx][ty] {
                ...
                let mut valid = false;
                if ty > fy && p.color == PieceColor::Red {
                    // red moves down
                    valid = true;
                }
                ...
                if ty < fy && p.color == PieceColor::Red && p.crowned {
                    // crowned red mv up
                    valid = true;
                }
                valid
